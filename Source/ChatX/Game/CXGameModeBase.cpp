...
void ACXGameModeBase::PrintChatMessageString(ACXPlayerController* InChattingPlayerController, const FString& InChatMessageString)
{
    ...
    if (IsGuessNumberString(GuessNumberString) == true)
    {
        ...
        if (IsValid(CXPS) == true && CXPS->CurrentGuessCount < 3)
        {
        ...
        {
            InChattingPlayerController->ClientRPCPrintChatMessageString(TEXT("You have reached the maximum number of attempts!"));
        }
    }
    else
    {
...
void ACXGameModeBase::JudgeGame(ACXPlayerController* InChattingPlayerController, int InStrikeCount)
{
    if (3 == InStrikeCount)
    {
        ...
        if (IsValid(CXPS) == true)
        {
            ...
            for (const auto& CXPlayerController : AllPlayerControllers)
            {
                if (IsValid(CXPlayerController) == true)
                {
                    CXPlayerController->NotificationText = FText::FromString(WinMessage);
    ...
    else
    {
        ...
        if (true == bIsDraw)
        {
            FString DrawMessage = TEXT("Game Over! It's a draw!");
            for (const auto& CXPlayerController : AllPlayerControllers)
            {
                if (IsValid(CXPlayerController) == true)
                {
                    CXPlayerController->NotificationText = FText::FromString(DrawMessage);
                    CXPlayerController->ClientRPCPrintChatMessageString(DrawMessage);
                }
            }
            ResetGame();
        }
